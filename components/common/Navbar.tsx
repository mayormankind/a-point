...
const Navbar = ({ userType }: { userType: 'client' | 'company' }) => {
    ...
    <div className="fixed bottom-0 w-full bg-white shadow-lg flex justify-around md:hidden">
      {userType === 'client' ? (
        <ul className='flex w-full justify-between'>
          {clientNavs.map(nav => (
            <li title={nav.label} onClick={()=>setClient(nav.label)} key={nav.label} className={`${client === nav.label ? 'border-t-4 w-full border-blue-600 transition font-semibold duration-300 text-gray-800 py-3' : 'py-4 text-gray-600'} px-2`}>
              <Link href={nav.ref}>
        ...
        <ul className='flex justify-between w-full'>
          {companyNavs.map(nav => (
            <li title={nav.label} onClick={()=>setCompany(nav.label)} key={nav.label} className={`${company === nav.label ? 'border-t-4 border-blue-600 transition font-semibold duration-300 text-gray-800' : 'text-gray-600'}`}>
              <Link href={nav.ref}>
                <div className="flex flex-col items-center">
                    <IconButton aria-label={nav.label} icon={company === nav.label ? nav.iconActive : nav.icon} variant='ghost' size='22px' className='h-8 w-8'/>
                </div>
              </Link>
            </li>
          ))}
